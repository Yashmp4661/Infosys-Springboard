...
function EditPolicyModal({ policy, onClose, onPolicyUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Same validation as AddPolicyModal
    ...
    if (!formData.benefits.trim() || !formData.exclusions.trim()) {
      setError('Benefits and exclusions are required');
      return;
    }
    ...
    try {
      ...
      await api.put(`/provider-admin/policies/${policy.id}`, policyData);
      ...
      const errorMessage = err.response?.data?.error || 'Failed to update policy. Please try again.';
      ...
      <div className="modal-content large-modal" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          ...
          <div className="modal-actions">
            ...
            >
              {loading ? 'Updating Policy...' : success ? 'Policy Updated!' : 'Update Policy'}
            </button>
          </div>
        </form>
      </div>
